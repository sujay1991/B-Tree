	...
	//BTLeafPage newRootPage=new BTLeafPage(headerPage.get_keyType());	
		if(headerPage.get_rootId().pid==INVALID_PAGE)
		{
                  BTLeafPage newRootPage=new BTLeafPage(headerPage.get_keyType());
                    ...
                    System.out.println("inside first insert");                     
                    ...
                    if(newRootEntry!=null)
                    {
                            ...
                            newRootPage.insertKey(key,newRootPage.getCurPage() );

                            newRootPage.setPrevPage(headerPage.get_rootId());
                           unpinPage(newRootPage.getCurPage());
                           updateHeader(newRootPage.getCurPage());
                           System.out.println("new index page created");
                    }
                    else
                         System.out.println("failed to insert index ");                  
                    System.out.println("end of first insert "+count);
                    count++;
...

	{
		...
		Page page=new Page();
		 ...
		 page = pinPage(currentPageId);
                ...
                System.out.println("inside _insert()");  
		...
		{        
                        ...
                        System.out.println("inside Index condition");                                                                                              unpinPage(currentIndexPageId);
                        if(upEntry==null)
                        {
                            return null;
                        }
                        ...
                        else
                        {
                            ...
                            for(tmpkeyDataEntry=currentIndexPage.getFirst(delRID);tmpkeyDataEntry!=null;tmpkeyDataEntry=currentIndexPage.getFirst(delRID))
                            {
                                    ...
                                    for(tmpkeyDataEntry=currentIndexPage.getFirst(delRID); newIndexPage.available_space()!=currentIndexPage.available_space(); tmpkeyDataEntry=currentIndexPage.getNext(delRID))
                                    {
                                        ...
                                        BT.getKeyDataLength(key, NodeType.INDEX);
                ...
                if(currentPage.getType()==NodeType.LEAF) 
		{
			...
			//BTIndexPage currentIndexPage=new BTIndexPage(page,headerPage.get_keyType());
                        System.out.println("inside leaf page condition");
			PageId currentLeafPageId=currentLeafPage.getCurPage(); 
                       //upEntry = new KeyDataEntry(key,rid);
                        //upEntry=_insert(key,rid,currentLeafPageId);
			//pinPage(currentLeafPageId);
			if(currentLeafPage.available_space()>=BT.getKeyDataLength(upEntry.key, NodeType.LEAF))
			{
				System.out.println("start ofif loop");
				currentLeafPage.insertRecord(upEntry.key,((LeafData)upEntry.data).getData() );
				//currentIndexPage.insertKey(upEntry.key, ((IndexData)upEntry.data).getData());
				unpinPage(currentLeafPageId);
                                System.out.println("inserting record on currentLeafPage");                               
                               				
			}			
			else
			{
				BTLeafPage newLeafPage=new BTLeafPage(page,headerPage.get_keyType());
				 ...
				 for(tmpkeyDataEntry=currentLeafPage.getFirst(delRID);tmpkeyDataEntry!=null;tmpkeyDataEntry=currentLeafPage.getFirst(delRID))
					{
						...
						/*for(tmpkeyDataEntry=currentLeafPage.getFirst(delRID); newLeafPage.available_space()!=currentLeafPage.available_space(); tmpkeyDataEntry=currentIndexPage.getNext(delRID))
						{
